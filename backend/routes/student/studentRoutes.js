...
router.post("/register", async (req, res) => {
  const { name, email, password, mobile, city, stream } = req.body;
  ...
  try {
    ...
    await pool.query(
      "INSERT INTO students (name, email, password, mobile, city, stream_id) VALUES (?, ?, ?, ?, ?, ?)",
      [name, email, hashed, mobile, city, stream || null]
    );
...
router.get("/me", async (req, res) => {
  if (!req.session.studentId) return res.status(401).json({ error: "Not logged in" });
  const [rows] = await pool.query("SELECT id,name,email,stream_id FROM students WHERE id=?", [
...
router.get("/tests/subjects/:stream_id", async (req, res) => {
  ...
  try {
    ...
    if (!test) return res.status(404).json({ error: "No test found for this stream" });
...
router.get("/tests/:testId/questions", async (req, res) => {
  ...
  try {
    ...
    const savedMap = Object.fromEntries(saved.map((r) => [r.question_id, r.answer]));
...
router.post("/tests/:testId/submit", async (req, res) => {
  ...
  try {
    ...
    if (!answers.length) return res.status(400).json({ error: "No answers found to submit." });

    const total = answers.length;
    const correct = answers.filter(
      (a) =>
        a.answer &&
        a.correct_answer?.trim().toLowerCase() === a.answer?.trim().toLowerCase()
    ).length;
