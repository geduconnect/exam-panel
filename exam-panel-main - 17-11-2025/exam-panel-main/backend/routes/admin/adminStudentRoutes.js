...
router.post("/", async (req, res) => {
  ...
  if (!name || !email || !mobile || !city || !school || !section || !stream_id)
  ...
  try {
    ...
    // Insert student
    await pool.query(
      "INSERT INTO students (name, email, mobile, city, school, section, stream_id) VALUES (?, ?, ?, ?, ?)",
      [name, email, mobile, city, stream_id, school, section]
    );
...
router.put("/:id", async (req, res) => {
  ...
  try {
    await pool.query(
      "UPDATE students SET name=?, email=?, mobile=?, city=?, stream=?,school=?, section=?, WHERE id=?",
      [name, email, mobile, city, stream_id, school, section, id]
    );
